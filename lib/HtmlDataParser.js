  ...
  async parse() {
    ...
    try {
      ...
      saxStream.on("opentag", function (node) {
        ...
        switch (node.name) {
          case "TABLE":
            tableFound = self.options.id ? (node.attributes[ "ID" ] === self.options.id) : headingFound;
            break;
          case "TR":
            row = [];
            break;
      ...
      saxStream.on("closetag", function (tag) {
        ...
        switch (tag) {
          case "TABLE":
            if (tableFound)
              tableFound = headingFound = false;
            tableDone = true;
            break;
          case "TR":
            if (tableFound && row.length > 0)
              self.output(row);
            break;
          case "TH":
          case "TD":
      ...
      saxStream.on("text", function (s) {
        ...
        if (tagOpen.H1 || tagOpen.H2 || tagOpen.H3 || tagOpen.H4 || tagOpen.H5 || tagOpen.H6) {
          headingText = s;
          if (self.options.heading === headingText)
            headingFound = true;
        }

        if (headingFound && tableFound && (tagOpen.TH || tagOpen.TD))
