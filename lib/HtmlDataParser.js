...
module.exports = class HtmlDataParser extends EventEmitter {
   ...
   * @param {URL|string}        options.url - the URL or local file name of the .html
   * @param {TypedArray|string} options.data - HTML file data as an array, instead of using url
   * @param {string}   [options.heading] - HTML section heading where data is located, default: none
   * @param {string}   [options.stopHeading] - HTML section heading after data table, default: none
  ...
  async parse() {
    ...
    // parsing properties
    let headingFound = Object.hasOwn(this.options, "heading") ? false : true;
    let tableFound = headingFound;
    let tableDone = false;
    ...
    try {
      ...
      const rs = fs.createReadStream(this.options.url);
